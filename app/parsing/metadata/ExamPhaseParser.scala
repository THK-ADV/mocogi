...
object ExamPhaseParser {
  ...
  private[parsing] def parser(using phases: Seq[ExamPhase]): Parser[NonEmptyList[ExamPhase]] =
    multipleValueParser(key, (p: ExamPhase) => s"$prefix${p.id}")(phases.sortBy(_.id).reverse).option
      .map(_.getOrElse(List(ExamPhase.none)))
      .nel()

  private[parsing] def raw: Parser[NonEmptyList[String]] =
