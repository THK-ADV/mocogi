...
final class ArtifactRoutesSpec extends AnyWordSpec with Matchers {
  "artifact routes" should {
    "identify their resources by PO only" in {
      ...
      routes.ModuleCatalogController.getPreview("inf_inf2").url shouldBe "/moduleCatalogs/preview/inf_inf2"
      routes.ModuleCatalogController.publish("inf_inf2").url shouldBe "/moduleCatalogs/inf_inf2"
      ...
      routes.ExamListsController.publish("inf_inf2").url shouldBe "/examLists/inf_inf2"
