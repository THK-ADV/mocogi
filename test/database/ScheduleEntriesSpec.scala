  ...
  test("by ids") {
    assertSnapshot("database/expected/schedule_entries/by_ids.txt") {
      sql"""SELECT jsonb_pretty((
            SELECT
              coalesce(
                jsonb_agg(entry ORDER BY (entry ->> 'id')::uuid),
                '[]'::jsonb
              )
            FROM
              jsonb_array_elements(schedule.get_schedule_entries(ARRAY['db711fad-39b5-4b72-8d4d-8d0c992f8e3f'::uuid,'20a2523d-f8c0-4893-b5f8-a72c603e9c0e'::uuid,'6ff26c44-41d9-49b7-bd68-7187b516e369'::uuid,'29e5d08a-0f29-49d9-8607-9edaf14346de'::uuid,'fd491392-bcd4-48cd-a2cd-7b4e5fb6e64c'::uuid])::jsonb) AS entries(entry)
          ))::text"""
        .as[String]
        .head
    }
  ...
  test("from 6. to 11. Apr. 2026") {
    assertSnapshot("database/expected/schedule_entries/block-week-sose-26.txt") {
      sql"""SELECT jsonb_pretty((
            ...
            FROM
              jsonb_array_elements(schedule.get_schedule_entries(
                TIMESTAMP '2026-04-06 00:00:00',
                TIMESTAMP '2026-04-11 00:00:00'
              )::jsonb) AS entries(entry)
          ))::text"""
        .as[String]
        .head
    }
